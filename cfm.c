...
cfm_tensor *cfm_tensor_from(cfm_string name, cfm_dtype dtype,
    ...
    //t->numel = (sizeof(data)/element_size);
    t->requires_grad = requires_grad;
    return t;
}
...
cfm_tensor *cfm_tensor_randn(cfm_string name, cfm_dtype dtype, 
    ...
    if (t->dtype == CFM_FLOAT32) {
        ...
        for (size_t i = 0; i < t->numel; ++i) {
            ...
            d[i] = sqrtf(-2.f*log((float)u1)) * cosf(_2_PI*u2);
        }
    } else {
        double *d = t->data;
        for (size_t i = 0; i < t->numel; ++i) {
            ...
            d[i] = sqrt(-2.f*log(u1)) * cos(_2_PI*u2);
        }
    }
    return t;
}
...
void cfm_tensor_print_raw(const cfm_tensor *t, cfm_print_mode pm) {
    ...
    switch (pm) {
        case CFM_H_PRINT:
            for (size_t i = 0; i < t->numel; ++i) {
                if (i == t->numel-1) printf("%f)\n", cfm_tensor_get_element(t, i));
                else printf("%f, ", cfm_tensor_get_element(t, i));
            }
            break;
        case CFM_V_PRINT:
            printf("\n");
            ...
            for (size_t i = 0; i < t->numel; ++i) {
                if (i == t->numel-1) printf("%*s%f)\n", (int)indent, "", cfm_tensor_get_element(t, i));
                else printf("%*s%f,\n", (int)indent, "", cfm_tensor_get_element(t, i));
            }
            break;
    }
